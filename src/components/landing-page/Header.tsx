                ...
                <div className="container mx-auto px-6 flex flex-col space-y-4">
                    {navLinks.map((link) => (
                        <Link
                            key={link.label}
                            to={link.href}
                            className="text-primary/70 hover:text-primary font-medium text-lg py-2"
                            onClick={() => setIsMenuOpen(false)}
                        >
                            {link.label}
                        </Link>
                    ))}
                    <Button className="bg-primary hover:bg-primary/90 text-white rounded-full w-full h-12 font-semibold mt-4">
                        Request a Demo
                    </Button>
                </div>
            </div>
