    ...
    return (
        <DashboardLayout variant="super-admin">
            ...
            <DashboardPageContent className="space-y-3 lg:space-y-4">
                {/* Top 4 KPIs */}
                <MetricCards metrics={adminMetricsData[selectedPeriod].slice(0, 4)} />

                {/* Clearance Rate + Active Marketers side by side */}
                <div className="grid grid-cols-2 gap-3 lg:gap-4">
                    {/* Clearance Rate — last metric in the array */}
                    {adminMetricsData[selectedPeriod].slice(4).map((metric) => (
                        <Card key={metric.title} className="relative overflow-hidden">
                            <div className={`absolute left-0 right-0 top-0 h-2 ${metric.topBarClass}`} />
                            ...
                            <CardContent className="p-3 pt-0 lg:p-6 lg:pt-0">
                                <div className="space-y-1.5 lg:space-y-2">
                                    ...
                                    <p className="text-[10px] text-muted-foreground lg:text-xs">{metric.period}</p>
                                </div>
                            </CardContent>
                        </Card>
                    ))}
