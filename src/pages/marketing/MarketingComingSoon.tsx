...
import { marketerNavConfig } from "@/config/navigation"

const MarketingComingSoon = () => {
    const location = useLocation()
    const navItems = [...marketerNavConfig.mainItems, ...marketerNavConfig.bottomItems]
    const activeItem = navItems.find((item) => location.pathname.startsWith(item.path))
    const title = activeItem?.label ?? "Marketing"
                ...
                <div className="flex flex-col items-center justify-center rounded-2xl border border-border bg-white py-20 text-center">
                    ...
                    <div className="mt-6 max-w-sm space-y-2">
                        ...
                        <p className="text-sm leading-relaxed text-muted-foreground">
                            The {title} workspace is being built. The Overview and Referrals
                            sections are ready to explore in the meantime.
                        </p>
                    </div>
                </div>
            </DashboardPageContent>
